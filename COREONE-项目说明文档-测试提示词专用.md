...
## 十一、用于DeepSeek的测试提示词模板

基于以上项目信息，以下是生成测试提示词的推荐模板：

```
你是一位资深QA工程师，正在为COREONE病理科耗材管理系统设计测试用例。

【项目背景】
COREONE是大型三甲医院病理科的耗材与试剂库存管理系统，支持：
- 181种物料（含96+种IHC抗体）的批号追踪和有效期管理
- 113个检测项目的BOM管理
- 6角色RBAC权限控制
- 采购→入库→出库→成本分析全链路

【技术栈】
后端: Express.js + TypeScript + SQLite(node:sqlite) + JWT
API: RESTful, Base URL: http://localhost:3001/api/v1
...
【角色权限】
admin: 全部
warehouse_manager: 入库/出库/库存/库位/预警
technician: 出库/项目/BOM/库存(读)
pathologist: 出库/项目/BOM/成本分析/库存(读)
procurement: 入库/分类/物料/供应商/采购单
finance: 成本分析/日志

【测试要求】
1. 按[模块]维度设计测试用例
2. 每个用例包含: 前置条件、测试步骤、预期结果、验证点
3. 必须覆盖: 正常流程、异常流程、边界条件、权限控制
4. 数据一致性验证: 库存=入库-出库+退货-报废
5. FIFO验证: 出库消耗最早过期批次
6. 安全测试: SQL注入、XSS、越权访问
