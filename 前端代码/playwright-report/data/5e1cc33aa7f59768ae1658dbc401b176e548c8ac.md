...
- Name: scenarios\technician-suite\view-outbound-records.spec.ts >> 技术员查看出库记录 - 出库列表路径 >> 路径1-步骤7: 验证API返回出库列表
- Location: e2e\scenarios\technician-suite\view-outbound-records.spec.ts:115:3
...
```ts
  18  |   const r = await apiFetch(token, 'GET', '/outbound?page=1&pageSize=1')
  ...
  39  | 
  40  |   outboundId = await getAnyOutboundId(adminToken)
  41  |   projectId = await getAnyProjectId(adminToken)
  42  | })
  43  | 
  44  | // ────────────────────────────────────────────
  45  | // 路径1: 查看出库列表 → 验证行可见 → 验证出库单号/项目名称
  46  | // ────────────────────────────────────────────
  47  | 
  48  | test.describe('技术员查看出库记录 - 出库列表路径', () => {
