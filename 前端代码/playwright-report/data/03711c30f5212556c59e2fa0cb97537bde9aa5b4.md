...
- Name: scenarios\warehouse-manager-suite\login.spec.ts >> 仓管员登录 - 验证侧边栏菜单路径 >> 路径3-步骤5: 验证侧边栏可见盘点管理
- Location: e2e\scenarios\warehouse-manager-suite\login.spec.ts:162:3
...
Locator: locator('text=盘点, a[href*="stocktaking"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=盘点, a[href*="stocktaking"]').first()
