...
- Name: scenarios\warehouse-manager-suite\login.spec.ts >> 仓管员登录 - 验证侧边栏菜单路径 >> 路径3-步骤6: 验证侧边栏可见调拨管理
- Location: e2e\scenarios\warehouse-manager-suite\login.spec.ts:170:3
...
Locator: locator('text=调拨, a[href*="transfer"]').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=调拨, a[href*="transfer"]').first()
