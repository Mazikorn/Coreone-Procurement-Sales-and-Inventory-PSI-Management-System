...
test.describe('角色权限 -> 查看角色列表', () => {
  ...
  test('ROLE-LIST-06. 权限：technician访问返回403', async ({ page }) => {
    ...
    // 前端权限守卫可能重定向到首页或显示403页面
    const msg = page.locator('text=/无权访问|403|Forbidden/i').first()
    const redirected = page.url() === `${FE_BASE}/` || page.url() === `${FE_BASE}`
    const has403 = await msg.isVisible().catch(() => false)
...
test.describe('角色权限 -> 编辑角色', () => {
  ...
  test('ROLE-EDIT-06. 并发：并发编辑同一角色', async ({ page }) => {
    ...
    if (!id) return
    const reqs = Array.from({ length: 2 }, () => apiFetch(token, 'PUT', `/roles/${id}`, { name: `concurrent-${Date.now()}` }).catch(() => ({ status: 500, data: null })))
    const results = await Promise.all(reqs)
    expect(results.some(r => [200, 403, 404, 409, 500].includes(r.status))).toBe(true)
  })
...
test.describe('角色权限 -> 角色详情弹窗', () => {
  ...
  test('ROLE-DETAIL-03. 正常用例：admin角色详情显示全部权限', async ({ page }) => {
    ...
    if (await detail.isVisible().catch(() => false)) {
      await detail.click(); await page.waitForTimeout(1000)
      await expect(page.locator('text=/全部权限|拥有系统全部权限|拥有全部权限/i').first()).toBeVisible()
      const close = page.locator('text=/关闭/i').first()
      if (await close.isVisible().catch(() => false)) await close.click()
    }
  })
