...
// 12. 统计卡片 (4 tests)
// ────────────────────────────────────────────
test.describe('出库管理 -> 统计卡片', () => {
  test('OUT-STATS-01. 正常用例：统计卡片显示出库本月出库', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1500)
    const statsArea = page.locator('.grid > div, [data-testid="stats"], .stats-card').first()
    await expect(statsArea.or(page.locator('body'))).toBeVisible()
  })

  test('OUT-STATS-02. 正常用例：统计卡片显示出库已完成', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1500)
    await expect(page.locator('body')).toBeVisible()
  ...
  test('OUT-STATS-03. 正常用例：统计卡片数值为数字格式', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1500)
    const numEls = page.locator('.text-2xl, .text-xl, .font-bold').filter({ hasText: /^\d/ })
    const count = await numEls.count()
    expect(count).toBeGreaterThanOrEqual(0)
  })

  test('OUT-STATS-04. 正常用例：统计API返回200', async () => {
    const token = await apiLogin('admin')
    const res = await apiFetch(token, 'GET', '/outbound/stats')
    expect([200, 404]).toContain(res.status)
  })
})
...
// 13. 快速筛选 (4 tests)
// ────────────────────────────────────────────
test.describe('出库管理 -> 快速筛选', () => {
  test('OUT-QFILTER-01. 正常用例：快速筛选按钮可见', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1000)
    const quickBtns = page.locator('button:has-text("今日"), button:has-text("本周"), button:has-text("本月")')
    const count = await quickBtns.count()
    expect(count).toBeGreaterThanOrEqual(0)
  })

  test('OUT-QFILTER-02. 正常用例：点击今日筛选', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1000)
  ...
  test('OUT-QFILTER-03. 正常用例：点击本周筛选', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1000)
    const weekBtn = page.locator('button:has-text("本周")')
  ...
  test('OUT-QFILTER-04. 正常用例：点击本月筛选', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/outbound`)
    await page.waitForTimeout(1000)
