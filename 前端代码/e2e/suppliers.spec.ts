...
// 11. 批量操作 (4 tests)
// ────────────────────────────────────────────
test.describe('供应商管理 -> 批量操作', () => {
  test('SUP-BATCH-01. 正常用例：可勾选供应商', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/suppliers`)
    await page.waitForTimeout(1000)
  ...
  test('SUP-BATCH-02. 正常用例：勾选后显示批量操作按钮', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/suppliers`)
    await page.waitForTimeout(1000)
    const checkboxes = page.locator('input[type="checkbox"], [role="checkbox"]')
    ...
    const batchBtn = page.locator('button:has-text("批量"), button:has-text("删除所选")')
    const count = await batchBtn.count()
    expect(count).toBeGreaterThanOrEqual(0)
  })

  test('SUP-BATCH-03. 正常用例：批量删除API可用', async () => {
    const token = await apiLogin('admin')
    const res = await apiFetch(token, 'POST', '/suppliers/batch-delete', { ids: [] })
    expect([200, 400, 404, 405]).toContain(res.status)
  })

  test('SUP-BATCH-04. 权限：procurement可操作批量', async () => {
    const token = await apiLogin('procurement')
    const res = await apiFetch(token, 'POST', '/suppliers/batch-delete', { ids: [] })
