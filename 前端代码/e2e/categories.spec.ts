...
async function loginAs(page: Page, role: RoleKey, retries = 3) {
...
test.describe('物料分类 -> 分类详情面板', () => {
  test('CAT-DETAIL-01. 正常用例：点击分类显示详情面板', async ({ page }) => {
    await loginAs(page, 'admin'); await page.goto(`${FE_BASE}/categories`); await page.waitForTimeout(1500)
    // 使用更精确的选择器：treeitem角色或包含分类名称的元素
    const catItem = page.locator('[role="treeitem"], [data-testid="category-item"]').first()
    if (await catItem.isVisible().catch(() => false)) { await catItem.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/基本信息|分类名称|分类编码|选择分类/i').first()).toBeVisible()
  })
  ...
  test('CAT-DETAIL-07. 正常用例：详情面板显示基本信息', async ({ page }) => {
    await loginAs(page, 'admin'); await page.goto(`${FE_BASE}/categories`); await page.waitForTimeout(1500)
    // 使用更精确的选择器：treeitem角色或包含分类名称的元素
    const catItem = page.locator('[role="treeitem"], [data-testid="category-item"]').first()
    if (await catItem.isVisible().catch(() => false)) { await catItem.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/基本信息|分类名称|选择分类/i').first()).toBeVisible()
  })
...
test.describe('物料分类 -> 状态管理', () => {
  ...
  test('CAT-STATUS-04. UI差异：分类目录显示分类名称', async ({ page }) => {
    await loginAs(page, 'admin'); await page.goto(`${FE_BASE}/categories`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/分类目录|分类总数|物料分类/i').first()).toBeVisible()
  })
...
test.describe('物料分类 -> 业务流程树', () => {
  test('BF-CAT-01. 主路径：创建一级→二级→三级分类', async ({ page }) => {
    ...
    if (pid1) {
      ...
      expect([200, 201, 409]).toContain(p2.status)
    }
  })
