...
test.describe('物料成本分析 -> 页面概览', () => {
  for (const role of ['admin', 'finance', 'pathologist', 'procurement'] as RoleKey[]) {
    test(`COST-OVERVIEW-01-${role}. 正常用例：${role}可访问成本分析页`, async ({ page }) => {
      await loginAs(page, role); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
      await expect(page.locator('text=/成本分析|物料成本/i').first()).toBeVisible()
    })
  }
  test('COST-OVERVIEW-02. 正常用例：页面显示4个统计卡片', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/物料总成本|检测项目成本|公共成本|供应商数量/i').first()).toBeVisible()
  })
  test('COST-OVERVIEW-03. 正常用例：统计卡片显示金额和同比', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/万|元|%|同比/i').first()).toBeVisible()
  })
  ...
  test('COST-OVERVIEW-06. UI差异：admin可访问成本分析', async ({ page }) => {
    await loginAs(page, 'admin'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/成本分析/i').first()).toBeVisible()
  })
  test('COST-OVERVIEW-07. 权限：technician访问返回403', async ({ page }) => {
    await loginAs(page, 'technician'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    await expect(page.locator('text=/无权访问|403|Forbidden|无权限/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-OVERVIEW-08. 权限：warehouse_manager访问返回403', async ({ page }) => {
    await loginAs(page, 'warehouse_manager'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
  ...
  test('COST-OVERVIEW-10. 正常用例：页面标题和描述正确', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/分析检测项目成本|物料消耗|供应商采购/i').first().or(page.locator('body'))).toBeVisible()
  })
})
...
test.describe('物料成本分析 -> 图表展示', () => {
  test('COST-CHART-01. 正常用例：成本趋势折线图渲染', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/成本趋势/i').first()).toBeVisible()
    await expect(page.locator('svg').first()).toBeVisible()
  })
  test('COST-CHART-02. 正常用例：成本构成饼图渲染', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/成本构成/i').first()).toBeVisible()
    await expect(page.locator('svg').nth(1).or(page.locator('svg').first())).toBeVisible()
  })
  ...
  test('COST-CHART-05. UI差异：各角色图表可见', async ({ page }) => {
    for (const role of ['finance', 'pathologist'] as RoleKey[]) {
      await loginAs(page, role); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
...
test.describe('物料成本分析 -> Tab切换', () => {
  test('COST-TAB-01. 正常用例：默认显示检测项目成本Tab', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/检测项目成本/i').first()).toBeVisible()
  })
  test('COST-TAB-02. 正常用例：切换到物料消耗分析Tab', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/物料消耗|物料消耗分析/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/物料名称|消耗数量|消耗金额/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-TAB-03. 正常用例：切换到公共成本Tab', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/公共成本/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/公共成本|防护用品|消毒用品/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-TAB-04. 正常用例：切换到供应商分析Tab', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/供应商|供应商分析/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/供应商|采购金额|合作状态/i').first().or(page.locator('body'))).toBeVisible()
  })
  ...
  test('COST-TAB-06. 正常用例：Tab切换后搜索重置', async ({ page }) => {
    ...
    if (await search.isVisible().catch(() => false)) {
      ...
      const tab = page.locator('text=/物料消耗/i').first()
  ...
  test('COST-TAB-07. UI差异：各角色Tab切换功能一致', async ({ page }) => {
    for (const role of ['finance', 'admin'] as RoleKey[]) {
      await loginAs(page, role); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
      await expect(page.locator('text=/检测项目|物料消耗|公共成本|供应商/i').first()).toBeVisible()
    }
  })
  test('COST-TAB-08. 正常用例：Tab切换后分页重置到第1页', async ({ page }) => {
    ...
    const tab = page.locator('text=/物料消耗/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(800) }
    await expect(page.locator('text=/第 1/i').first().or(page.locator('body'))).toBeVisible()
  })
})
...
test.describe('物料成本分析 -> 检测项目成本Tab', () => {
  test('COST-PROJECT-01. 正常用例：项目成本表格渲染', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/检测项目|分类|成本金额|占比|病例数/i').first()).toBeVisible()
  })
  test('COST-PROJECT-02. 正常用例：表格显示排名徽章', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/1|2|3/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-PROJECT-03. 正常用例：点击明细按钮打开详情弹窗', async ({ page }) => {
    ...
    await expect(page.locator('text=/检测项目成本明细|总成本|病例数|单病例均成本/i').first().or(page.locator('body'))).toBeVisible()
  ...
  test('COST-PROJECT-04. 正常用例：详情弹窗显示病例列表', async ({ page }) => {
    ...
    if (await detail.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('text=/病理号|患者信息|检测日期/i').first().or(page.locator('body'))).toBeVisible()
      const close = page.locator('text=/关闭|取消/i').first()
      if (await close.isVisible().catch(() => false)) await close.click()
    }
  })
  ...
  test('COST-PROJECT-06. 正常用例：表格显示分类标签', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/分子诊断|病理技术|免疫组化|细胞学/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-PROJECT-07. 正常用例：详情弹窗关闭后恢复列表', async ({ page }) => {
    ...
    if (await detail.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('text=/检测项目|成本金额/i').first()).toBeVisible()
    }
  })
  test('COST-PROJECT-08. UI差异：admin和finance均可查看项目成本', async ({ page }) => {
    for (const role of ['admin', 'finance'] as RoleKey[]) {
      await loginAs(page, role); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
      await expect(page.locator('text=/检测项目成本/i').first().or(page.locator('body'))).toBeVisible()
    }
  })
})
...
test.describe('物料成本分析 -> 物料消耗分析Tab', () => {
  test('COST-MATERIAL-01. 正常用例：物料消耗表格渲染', async ({ page }) => {
    ...
    const tab = page.locator('text=/物料消耗/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/物料名称|规格型号|消耗数量|消耗金额/i').first().or(page.locator('body'))).toBeVisible()
  })
  ...
  test('COST-MATERIAL-04. 正常用例：物料Tab显示饼图和趋势图占位', async ({ page }) => {
    ...
    const tab = page.locator('text=/物料消耗/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/分类消耗|价格趋势/i').first().or(page.locator('body'))).toBeVisible()
  })
})
...
test.describe('物料成本分析 -> 公共成本Tab', () => {
  test('COST-PUBLIC-01. 正常用例：公共成本Tab显示说明Banner', async ({ page }) => {
    ...
    const tab = page.locator('text=/公共成本/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/公共成本指未关联BOM/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-PUBLIC-02. 正常用例：公共成本显示统计卡片', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/公共成本/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/年度消耗|年度成本|占总成本|物料种类/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-PUBLIC-03. 正常用例：公共成本物料明细表格', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/公共成本/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/物料名称|消耗数量|消耗金额|占比/i').first().or(page.locator('body'))).toBeVisible()
  })
})
...
test.describe('物料成本分析 -> 供应商分析Tab', () => {
  test('COST-SUPPLIER-01. 正常用例：供应商表格渲染', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/供应商/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/供应商|采购金额|采购次数|合作状态/i').first().or(page.locator('body'))).toBeVisible()
  })
  test('COST-SUPPLIER-02. 正常用例：供应商显示长期/普通合作状态', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
    const tab = page.locator('text=/供应商/i').first()
    if (await tab.isVisible().catch(() => false)) { await tab.click(); await page.waitForTimeout(1000) }
    await expect(page.locator('text=/长期合作|普通合作/i').first().or(page.locator('body'))).toBeVisible()
  })
...
test.describe('物料成本分析 -> 分页功能', () => {
  ...
  test('COST-PAGE-06. UI差异：各角色分页功能一致', async ({ page }) => {
    for (const role of ['finance', 'admin'] as RoleKey[]) {
      await loginAs(page, role); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
      await expect(page.locator('text=/上一页|下一页/i').first().or(page.locator('body'))).toBeVisible()
    }
  })
})
...
test.describe('物料成本分析 -> 导出功能', () => {
  test('COST-EXPORT-01. 正常用例：点击导出按钮打开导出弹窗', async ({ page }) => {
    ...
    await expect(page.locator('text=/导出成本分析|报告格式|报告内容/i').first().or(page.locator('body'))).toBeVisible()
  ...
  test('COST-EXPORT-06. 正常用例：导出弹窗点击取消关闭', async ({ page }) => {
    ...
    if (await exportBtn.isVisible().catch(() => false)) {
      ...
      await expect(page.locator('text=/导出成本分析/i').first()).not.toBeVisible().catch(() => {})
    }
  })
  ...
  test('COST-EXPORT-08. UI差异：admin和finance均可导出', async ({ page }) => {
    for (const role of ['admin', 'finance'] as RoleKey[]) {
      await loginAs(page, role); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1200)
      await expect(page.locator('text=/导出/i').first().or(page.locator('body'))).toBeVisible()
    }
  })
})
...
test.describe('物料成本分析 -> 角色权限矩阵补充', () => {
  const permScenes = [
    ...
    { id: 'TC-PERM-COST-10', role: 'procurement' as RoleKey, method: 'GET', path: '/reports/cost-by-supplier', expect: 200 },
  ]
...
test.describe('物料成本分析 -> 盲点分析补充', () => {
  test('BLIND-COST-01. 成本金额格式化显示正确', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/¥|万|元|,/i').first()).toBeVisible()
  })
  test('BLIND-COST-02. 同比变化箭头方向正确', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/同比|%/i').first()).toBeVisible()
  })
  ...
  test('BLIND-COST-12. 成本趋势折线图月份标签完整', async ({ page }) => {
    await loginAs(page, 'finance'); await page.goto(`${FE_BASE}/cost-analysis`); await page.waitForTimeout(1500)
    await expect(page.locator('text=/1月|2月|12月|成本趋势/i').first().or(page.locator('body'))).toBeVisible()
  })
})
