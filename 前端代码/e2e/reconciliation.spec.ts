...
// ── 13. 修改病例弹窗 ──
test.describe('消耗对账 -> 修改病例弹窗', () => {
  test('RECON-EDIT-CASE-01. 正常用例：点击修改按钮弹窗可打开', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/reconciliation`)
    await page.click('text=按病理号查看')
    await page.waitForTimeout(1000)
    const editBtn = page.locator('button:has-text("修改"), text=修改').first()
  ...
  test('RECON-EDIT-CASE-02. 正常用例：弹窗可关闭', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/reconciliation`)
    await page.click('text=按病理号查看')
    await page.waitForTimeout(1000)
    ...
    if (await editBtn.isVisible().catch(() => false)) {
      await editBtn.click()
      await page.waitForTimeout(500)
      const closeBtn = page.locator('button:has-text("取消"), button:has-text("关闭"), button:has-text("×")')
  ...
  test('RECON-EDIT-CASE-03. 正常用例：修改API可用', async () => {
    const token = await apiLogin('admin')
    const res = await apiFetch(token, 'PUT', '/reconciliation/cases/test-case-id', {
      projectName: 'HE制片', operator: '测试修改',
    })
    expect([200, 400, 404]).toContain(res.status)
  })

  test('RECON-EDIT-CASE-04. 正常用例：修改病例API返回格式', async () => {
    const token = await apiLogin('admin')
    const res = await apiFetch(token, 'PUT', '/reconciliation/cases/test-case-id', {
      projectName: 'HE制片',
    })
  ...
  test('RECON-EDIT-CASE-05. 权限：technician无修改权限', async () => {
    const token = await apiLogin('technician')
    const res = await apiFetch(token, 'PUT', '/reconciliation/cases/test-case-id', {
      projectName: 'HE制片',
    })
  ...
  test('RECON-EDIT-CASE-06. 表单校验：弹窗表单校验', async ({ page }) => {
    await loginAs(page, 'admin')
    await page.goto(`${FE_BASE}/reconciliation`)
    await page.click('text=按病理号查看')
    await page.waitForTimeout(1000)
    ...
    if (await editBtn.isVisible().catch(() => false)) {
      ...
      const submitBtn = page.locator('button:has-text("确认"), button:has-text("保存"), button[type="submit"]')
