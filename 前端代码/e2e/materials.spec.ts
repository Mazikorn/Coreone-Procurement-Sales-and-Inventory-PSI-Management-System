...
test.describe('耗材管理 -> 切换启用停用', () => {
  ...
  test('MAT-STATUS-02. 异常用例：停用提交失败后关闭弹窗并刷新为原状态', async ({ page }) => {
    await loginAs(page, 'admin')
      ...
      if (url.pathname.endsWith('/categories')) {
        await route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            data: {
              list: [{ id: 'cat-status-fail', name: '状态失败分类', status: 'active' }],
              pagination: { total: 1, page: 1, pageSize: 999 },
            },
          }),
        })
    ...
    await page.route('**/api/v1/materials**', async (route) => {
      ...
      if (url.pathname.endsWith('/materials/mock-status-fail/check-status')) {
        checkCalled = true
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            data: {
              material: { id: 'mock-status-fail', code: 'STS-FAIL-001', name: '停用失败Mock物料' },
      ...
      if (url.pathname.endsWith('/materials/mock-status-fail') && request.method() === 'PUT') {
        updateCalled = true
        await route.fulfill({
          status: 409,
          contentType: 'application/json',
          body: JSON.stringify({
            success: false,
            error: { message: '物料状态已被其他业务变更，请刷新后重试', code: 'CONFLICT' },
          }),
        })
        return
      }
    ...
    await page.goto(`${FE_BASE}/materials`, { waitUntil: 'domcontentloaded' })
    await expect(page.getByText('停用失败Mock物料')).toBeVisible({ timeout: 15000 })

    const row = page.getByRole('row').filter({ hasText: '停用失败Mock物料' })
    await row.getByRole('button', { name: '停用', exact: true }).click()
    await expect(page.getByRole('heading', { name: '确定要停用该物料吗？' })).toBeVisible()
    await expect(page.getByText('未发现阻断影响，可以停用。')).toBeVisible()
    await page.getByRole('button', { name: '确认停用' }).click()

    await expect(page.getByRole('heading', { name: '停用物料' })).toHaveCount(0)
