...
- Name: users.spec.ts >> 用户管理 -> 盲点分析补充 >> BLIND-USER-06. 分页页码按钮样式
- Location: e2e\users.spec.ts:608:3
...
Locator: locator('text=/上一页|下一页|共.*条/i').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=/上一页|下一页|共.*条/i').first()
