...
const apiMap: Record<EntityType, (params: Record<string, unknown>) => Promise<any>> = {
...
export function useReferenceData(
  ...
  const [data, setData] = useState<Record<EntityType, ReferenceItem[]>>({
    materials: [],
    suppliers: [],
    locations: [],
    projects: [],
    ...
    try {
      const results = await Promise.all(
        entities.map(async (entity) => {
          ...
          if (!api) return [entity, [] as ReferenceItem[]] as const
          const res = await api({ page: 1, pageSize, status })
          return [entity, (res?.list || []) as ReferenceItem[]] as const
        })
      )
      setData(prev => {
        const next = { ...prev }
