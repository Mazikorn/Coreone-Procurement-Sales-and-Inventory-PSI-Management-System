...
interface Props {
  ...
  onChangeResponsiblePerson: (v: string) => void
  onChangeResponsibleDepartment: (v: string) => void
}

export function BatchScrapModal({
  open,
  items,
  scrapReason,
  scrapRemark,
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden max-h-[80vh] flex flex-col">
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">责任人（高价值必填）</label>
              <input
                value={responsiblePerson}
                onChange={e => onChangeResponsiblePerson(e.target.value)}
                placeholder="填写损耗责任人"
                className="w-full h-10 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:border-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">责任部门（高价值必填）</label>
              <input
                value={responsibleDepartment}
                onChange={e => onChangeResponsibleDepartment(e.target.value)}
                placeholder="填写责任部门"
                className="w-full h-10 px-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:border-blue-500"
              />
            </div>
