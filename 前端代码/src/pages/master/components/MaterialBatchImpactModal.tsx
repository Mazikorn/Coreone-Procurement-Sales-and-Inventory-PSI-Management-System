...
import { AlertTriangle, CheckCircle2, Trash2, Power } from 'lucide-react'
import { Modal } from '@/components/ui/Modal'
import type { MaterialBatchAction, MaterialBatchDeleteResult, MaterialBatchStatusResult } from '../hooks/useMaterialsPage'

interface Props {
  open: boolean
  action: MaterialBatchAction | null
  targetsCount: number
  deleteResults: MaterialBatchDeleteResult[]
  statusResults: MaterialBatchStatusResult[]
  checking: boolean
  submitting: boolean
  onClose: () => void
  onConfirm: () => void
...
export function MaterialBatchImpactModal({
  open,
  action,
  targetsCount,
  deleteResults,
  ...
  const title = isDelete
    ? blockedCount > 0 ? '无法批量删除物料' : '批量删除物料'
    : blockedCount > 0
      ? isDisabling ? '无法批量停用物料' : '无法批量启用物料'
      : isDisabling ? '批量停用物料' : '批量启用物料'
      ...
      <div className="space-y-4">
        <div className="flex items-start gap-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
          ...
          <div>
            <div className="text-sm font-semibold text-gray-900">
              已选择 {targetsCount} 个物料，{blockedCount > 0 ? `${blockedCount} 个存在阻断影响` : '未发现阻断影响'}
            </div>
            <div className="mt-1 text-sm text-gray-500">
              {isDelete
                ? '批量删除前会检查库存、BOM、出入库记录和库存流水；任一物料不可删除时整批不执行。'
                : isDisabling
                  ? '批量停用前会检查当前库存、库位库存和启用BOM引用；任一物料不可停用时整批不执行。'
                  : '批量启用前会检查分类、供应商和默认库位是否可用；任一物料不可启用时整批不执行。'}
            </div>
          </div>
        </div>
          ...
          {checking ? (
            ...
            <div className="divide-y divide-gray-100">
              {deleteResults.map(item => {
                ...
                return (
                  <div key={item.material.id} className="flex items-start justify-between gap-4 p-3">
                    <div>
        ...
        {blockedCount > 0 ? (
          <div className="rounded-md bg-amber-50 px-3 py-2 text-sm text-amber-700">
            存在阻断项，批量操作不会执行。请先处理对应库存、BOM 或业务记录引用。
          </div>
        ) : (
