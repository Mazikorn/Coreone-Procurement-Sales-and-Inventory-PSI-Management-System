...
const btnCls = 'inline-flex h-9 items-center gap-1.5 rounded-md border border-gray-200 bg-white px-3 text-[13px] font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50'
const btnPri = 'inline-flex h-9 items-center gap-1.5 rounded-md bg-blue-500 px-3.5 text-[13px] font-medium text-white transition-colors hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50'
...
export default function PartnerConfigPage() {
  ...
  const discard = useCallback(async () => {
    ...
    if (!window.confirm('放弃所有未保存改动？')) return
    const env = await partnerConfigApi.get(curId)
  ...
  const rollback = useCallback(async (toVersion: number) => {
    if (!curId) return
    if (!window.confirm(`回滚到 v${toVersion}？当前未保存改动将丢失，回滚本身也会记一条变更。`)) return
    try {
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <button className={btnCls} onClick={() => { if (dirty && !window.confirm('有未保存改动，确定返回？')) return; setView('list') }}><ArrowLeft className="h-4 w-4" />返回列表</button>
      ...
      <div className="mb-4 flex flex-wrap gap-1 border-b border-gray-200">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`h-9 rounded-t-md px-3 text-[13px] font-medium transition-colors ${tab === t.key ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-800'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {detailLoading ? (
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
          ...
          {tab === 'lines' && <LinesTab cfg={cfg} patch={patch} />}
...
function LinesTab({ cfg, patch }: { cfg: PartnerConfig; patch: PatchFn }) {
      ...
      {cfg.lines.map((l, i) => (
        <div key={l.key} className="rounded-md border border-gray-200 p-3">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <input className="h-8 w-44 rounded-md border border-gray-200 bg-white px-2 text-[13px] font-medium outline-none focus:border-blue-500" value={l.name} onChange={(e) => patch((c) => { c.lines[i].name = e.target.value })} />
            <button role="switch" aria-checked={l.on} tabIndex={0}
              ...
              className={`inline-flex h-5 w-9 items-center rounded-full px-0.5 transition-colors ${l.on ? 'bg-blue-500' : 'bg-gray-300'}`}>
            ...
            <div className="inline-flex overflow-hidden rounded-md border border-gray-200 text-[12px]">
              {(['in', 'out'] as const).map((s) => (
                <button key={s} onClick={() => patch((c) => { c.lines[i].scope = s })}
            ...
            <button className="ml-auto text-gray-300 hover:text-red-500" aria-label="删除业务线" onClick={() => { if (window.confirm(`删除业务线「${l.name}」？`)) patch((c) => { c.lines.splice(i, 1) }) }}><X className="h-4 w-4" /></button>
          </div>
      ...
      {changes.map((c) => (
        <div key={c.version} className="border-l-2 border-gray-200 pl-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={`rounded-full px-1.5 py-0.5 text-[11px] ${c.kind === 'rollback' ? 'bg-purple-50 text-purple-600' : c.kind === 'seed' ? 'bg-gray-100 text-gray-500' : 'bg-blue-50 text-blue-600'}`}>{c.kind === 'rollback' ? '回滚' : c.kind === 'seed' ? '建档' : '编辑'}</span>
            {c.version > 1 && <button className="ml-auto inline-flex items-center gap-1 text-[12px] text-purple-600 hover:underline" onClick={() => onRollback(c.version)}><History className="h-3.5 w-3.5" />回滚到此版本</button>}
          </div>
          {c.diffs?.length > 0 && (
