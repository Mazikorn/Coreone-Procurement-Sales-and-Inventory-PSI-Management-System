...
const yuan = (n: number) => '¥' + Math.round(n || 0).toLocaleString('zh-CN')
...
export default function HospitalPnLDashboard() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const params = serviceMonth ? { serviceMonth } : undefined
      const [ov, cs] = await Promise.all([
        partnerPnlApi.overview(params),
      ...
      setRows(list)
      setFlagged(cs?.list || [])
      const top = selected || list.slice().sort((a, b) => b.labRevenueTotal - a.labRevenueTotal)[0]?.partnerId || ''
      setSelected(top)
      setTrend(top ? await partnerPnlApi.trend(top) : [])
    } catch {
      toast.error('加载院级 P&L 失败')
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">按合作医院的财务实收 → 实验室收入 → ABC 成本 → 院级毛利</p>
        </div>
      ...
      {loading ? (
        ...
        <div className={cn(CARD, 'p-10')}>
          <EmptyState icon={Building2} title="本期暂无院级 P&L 数据" description="先导入 LIS 病例与财务收费单据，回填成本后这里即可看到每家医院的毛利。" />
        </div>
      ) : (
        <>
          {/* KPI */}
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <Kpi icon={Wallet} label="ABC 成本" value={yuan(kpi.cost)} sub="按医院上卷" />
          ...
          <div className={cn(CARD, 'overflow-hidden')}>
            <div className="flex items-center px-4 py-3 border-b border-slate-100">
              <h2 className={cn('text-sm font-semibold', INK)}>院级 P&amp;L · 负毛利置顶</h2>
              <div className="ml-auto flex items-center gap-3 text-xs">
                ...
                <span className="inline-flex items-center gap-1 text-slate-400"><Info className="w-3.5 h-3.5" /> benchmark 未病种校正</span>
              </div>
            </div>
              ...
              <table className="w-full text-sm tabular-nums">
                <thead>
                  <tr className="text-xs text-slate-400">
                    ...
                    <th className="text-right font-medium px-4 py-2.5">ABC 成本</th>
                  ...
                  {rows.map((r) => {
                    ...
                    return (
                      <tr key={r.partnerId} onClick={() => pickPartner(r.partnerId)}
                        className={cn('border-t border-slate-50 cursor-pointer transition-colors',
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className={cn(CARD, 'p-4')}>
              <h2 className={cn('text-sm font-semibold mb-3', INK)}>负毛利 case 筛查</h2>
              {flagged.length === 0 ? (
                <div className="h-[260px] flex items-center justify-center text-sm text-slate-400">本期无负毛利 case</div>
              ) : (
