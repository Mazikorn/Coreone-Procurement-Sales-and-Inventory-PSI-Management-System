...
const getListPayload = <T,>(response: any): T[] =>
  response?.list || response?.items || response?.data?.list || response?.data?.items || response?.data || response || []
...
export function ActivityCenterList() {
  ...
  const loadActivityCenters = async () => {
    try {
      setLoading(true)
      const data = await abcApi.getActivityCenters()
      setActivityCenters(getListPayload<ActivityCenter>(data))
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingCenter) {
        await abcApi.updateActivityCenter(editingCenter.id, formData)
      } else {
        await abcApi.createActivityCenter(formData)
      }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await abcApi.deleteActivityCenter(deletingId)
      toast.success('删除成功')
      loadActivityCenters()
    } catch (error) {
