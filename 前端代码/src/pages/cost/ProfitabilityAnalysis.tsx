...
export function ProfitabilityAnalysis() {
  ...
  const loadProfitabilityData = async () => {
    try {
      setLoading(true)
      const result: any = await abcApi.getProfitability(
        projectType !== 'all' ? { projectType } : undefined
      )
      setData(result?.list || result?.items || result?.data?.list || result?.data?.items || result?.data || result || [])
    } catch (error) {
      console.error('Failed to load profitability data:', error)
      toast.error('加载盈利性分析失败')
    } finally {
      setLoading(false)
    }
  }
