...
const getTotalPurchaseValue = (stat: DepreciationStat) =>
  stat.totalPurchaseValue ?? stat.totalPurchasePrice ?? 0

const getTotalDepreciation = (stat: DepreciationStat) =>
  stat.totalDepreciation ?? stat.totalAnnualDepreciation ?? 0
...
export default function EquipmentDepreciationStats() {
  ...
  const chartData = useMemo(() =>
    stats.map(s => ({
      name: s.typeName || '未分类',
      totalDepreciation: getTotalDepreciation(s),
      equipmentCount: s.equipmentCount || 0,
      totalPurchaseValue: getTotalPurchaseValue(s),
    })),
    [stats]
  )
  ...
  const summaryCards = [
    ...
    {
      label: '总购置价值',
      value: summary?.totalPurchaseValue ?? stats.reduce((s, d) => s + getTotalPurchaseValue(d), 0),
      icon: DollarSign,
    ...
    {
      label: '累计折旧',
      value: summary?.totalDepreciation ?? stats.reduce((s, d) => s + getTotalDepreciation(d), 0),
      icon: TrendingUp,
    ...
    },
    {
      label: '净值',
      value: (summary?.totalPurchaseValue ?? stats.reduce((s, d) => s + getTotalPurchaseValue(d), 0))
        - (summary?.totalDepreciation ?? stats.reduce((s, d) => s + getTotalDepreciation(d), 0)),
      icon: DollarSign,
  ...
  const handleExport = () => {
    ...
    const rows = stats.map(s => [
      s.typeName || '未分类',
      s.equipmentCount || 0,
      getTotalPurchaseValue(s),
      getTotalDepreciation(s),
      getTotalPurchaseValue(s) - getTotalDepreciation(s),
    ])
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              ...
              stats.map(s => {
                const purchaseValue = getTotalPurchaseValue(s)
                const depreciationValue = getTotalDepreciation(s)
                const netValue = purchaseValue - depreciationValue
                const depreciationRate = purchaseValue
                ...
                return (
                  <tr key={s.typeId || s.typeName} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 text-right font-mono">{formatCurrency(purchaseValue)}</td>
                    <td className="px-4 py-3 text-sm text-gray-900 text-right font-mono">{formatCurrency(depreciationValue)}</td>
