...
| 当前工作树关键文件完整 | `app.ts` 引用的路由文件均存在 | ⚠️ 待目标分支构建/启动确认 |

### 2. Bug 状态

| 检查项 | 标准 | 当前状态 |
|--------|------|----------|
| 无未确认 P0 Bug | 0 个 | ⚠️ 待重新清点；旧待办含 23 个 P0，不可直接发布 |
| 无未确认 P1 Bug | 0 个 | ⚠️ 待重新清点；旧待办含 22 个 P1 |
...
| 检查项 | 标准 | 当前状态 |
|--------|------|----------|
| 核心 E2E 通过 | 当前存在的核心 spec 通过 | ⚠️ 待本轮重跑验证 |
| 完整 E2E 通过率 | ≥95% | ⚠️ 待重跑验证；当前仅确认 26 个 spec 文件可见 |
| 后端单元测试通过 | 100% | 待确认 |
| 前端单元测试通过 | 100% | 待确认 |
...
| 角色测试已覆盖 | E2E roles.spec.ts | ⚠️ 文件可见，待运行确认 |
...
| SQL 参数化查询 | 无字符串拼接 | ⚠️ 待专项审计 |
| 错误消息不泄露内部细节 | 统一错误码 | ⚠️ 待专项审计 |
| 认证/授权在每个路由验证 | requireRole | ⚠️ 待目标分支 `app.ts`、routes、权限测试三方核对 |
...
**当前建议结论（2026-06-11）**：不可仅凭治理文档发布。需先在目标分支完成后端构建/启动、核心 E2E、权限矩阵核对和 PM 验收记录更新。
