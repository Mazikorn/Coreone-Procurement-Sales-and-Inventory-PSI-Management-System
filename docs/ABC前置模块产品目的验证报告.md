...
但当前版本管理更像“同一 BOM 行上的版本号递增”，历史留痕不足。2026-06-16 已补齐首版出库来源快照、BOM 版本表和变更影响统计：BOM 出库与重算会在成本快照中保存当时 BOM 版本、用量明细和收费映射规则；BOM 创建/更新会写入 `bom_versions` 快照，详情页可查看版本历史、关键差异、生效范围和影响出库数；选择“追溯重算”后会自动对未关账受影响月份创建重算任务；已关账月份可通过关账后调整单承接。病例阶梯聚合已补齐取消回退和非最新历史单重排首版，能保证剩余单据明细金额与病例聚合组总额一致。

建议：

- 出库时继续保存完整 BOM 来源快照，后续可升级为 `bom_snapshot_id`。
- BOM 编辑继续新增版本记录和版本差异提示，不覆盖历史版本的成本核算依据。
- 对账修正 BOM 用量时，必须区分“从下月生效”和“追溯重算”；当前已能记录选择、提示受影响历史出库范围，并自动重算未关账月份。
...
- 2026-06-16 已新增 `bom_versions` 首版：创建/更新 BOM 会保存版本快照、结构化差异、生效范围和历史出库影响统计。

不足：

- BOM 更新仍会替换当前 `bom_items`，虽然已有版本快照、生效范围留痕、未关账月份自动重算和已关账调整单首版，但复杂历史差异仍需要在后续报表中强化说明。
- BOM-收费标准映射已完成首版独立配置，并已通过 LIS 病例号进入标准 BOM 出库流程打通到对账链路；病例阶梯聚合取消回退和非最新历史单重排已完成首版。
- 对账修正如果直接更新 `bom_items.usage_per_sample`，仍需要复用生效范围选择，并接入当前未关账月份自动重算与已关账调整单流程。

判断：传统 BOM 目的已基本满足；ABC 前置目的已具备版本留痕、生效范围、影响统计、未关账追溯重算、已关账调整单和病例聚合历史回放首版。

建议：

- BOM 变更时要求选择“仅未来生效/追溯重算”。
- 出库继续保存 BOM 来源快照，后续可升级为版本快照 ID 关联。
- 收费标准映射继续与病例、LIS、对账链路打通。
...
- 2026-06-16 已补齐真实 CSV 导出首版：可按项目、物料、病例、日志导出对账数据，页面“导出报表”按当前 Tab 下载。
- 对账修正直接改 `bom_items`，如果没有生效期和版本策略，会污染历史。
- LIS 导入的 project_id 验证曾被列为问题，说明病例和项目映射不稳。
- 2026-06-16 已补齐对账差异异常化和页面触发首版：项目物料对账审计会把 warn/danger 差异写入 `cost_exceptions`，差异恢复正常后自动关闭开放异常；`/reconciliation` 页面已恢复路由，项目展开后可触发“审计差异”。
- 2026-06-16 已补齐 LIS 病例进入标准出库流程首版：BOM 出库支持只传 `caseNo` 反查 LIS 病例并自动带出项目/BOM；出库表单可选择 LIS 病例，自动填入病例号、项目、BOM 和样本数。

判断：对账雏形存在；ABC 前置目的不足。

建议：

- 对账差异继续进入 `cost_exceptions`，形成处理状态，并在异常中心闭环处理。
- 修正 BOM 时要求选择“仅未来生效/追溯重算”。
- 对账导出继续扩展为 Excel/PDF 模板，用于财务复核归档。
...
1. 成本看板、异常中心、出库成本状态、成本池入口已完成首版，能支撑期间归集、重算、关账前异常校验和基础审计。
2. 收费映射已完成独立配置入口：支持一 BOM 多收费项、数量系数、按出库/病例聚合、规则预览和完整性检查。
3. 未配置收费映射的 BOM 已能进入成本异常台账：完整性审计可批量发现缺口，BOM 出库路径也会即时标记 `cost_exception` 并写入 `missing_fee_mapping` 待处理异常；配置补齐后可通过完整性审计自动关闭异常。
4. 病例号前端采集、出库详情回显、仓管项目/BOM 只读选择、BOM 出库来源快照已完成首版。
5. BOM 版本表、版本差异 UI、生效范围选择、历史影响统计和未关账追溯自动重算已完成首版：创建/更新 BOM 会写入版本快照，详情页可查看版本历史、关键用量差异、生效范围和影响出库数；选择追溯后会自动创建重算任务。
6. 病例聚合收费已补取消回退和历史重排：取消 BOM 出库时会回退病例收费组并重新计算累计收费；取消非最新历史单后会重排剩余 ABC 明细的收费、利润和来源快照。
7. 对账差异自动异常化、页面触发入口和真实导出已完成首版：项目物料对账审计可写入/关闭成本异常，页面可触发审计并导出当前 Tab CSV。
8. 关账后调整单已完成首版：已关账期间禁止重算但可创建调整单，审核通过后进入看板调整额、调整后利润和 ABC 导出，并写入审计日志。
9. LIS 病例进入标准出库流程已完成首版：病例号可从 LIS/对账链路进入 BOM 出库，并保留出库记录、ABC 明细和来源快照链路。
10. 当前产品目的链路的一版闭环已覆盖到 LIS 病例、BOM 出库、ABC 明细、病例聚合、对账、异常、重算、关账和调整；后续重点应转向全量验收、报表解释性和真实数据迁移策略。
