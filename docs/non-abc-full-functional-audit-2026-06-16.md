...
- 对账页面导入、导出、日期筛选、日志筛选、病例编辑后的 summary/project 刷新，以及 BOM 修正成功后刷新失败解释已阶段性保护；BOM 修正后重新审计成本异常的提示作为下一独立批次处理，避免在刷新失败解释批次中扩大范围。

## 三百三十二、批次 377: BOM 修正后必须提示重新审计成本异常

**发现的问题**

- 本轮继续复核 ABC 之外的 LIS/对账页面，聚焦“页面提示必须准确说明后续成本异常同步动作”不变量。
- 后端事实链显示：BOM 修正日志接口只更新 BOM 用量和修正日志；成本异常台账的创建、更新和关闭发生在 `POST /api/v1/reconciliation/projects/:id/materials/audit`。
- 页面旧成功提示只有 `BOM用量已修正`，容易让用户误以为对账差异和成本异常台账已经同步，而实际上还需要重新执行“审计差异”。
- 本批不自动触发审计，避免把“修正 BOM”扩展成额外成本异常写入副作用；只把现有系统真实同步入口明确告诉用户。

**已完成修复**

- `前端代码/src/pages/reconciliation/hooks/useReconciliationPage.ts`
  - BOM 修正成功提示改为 `BOM用量已修正，请重新审计差异以同步成本异常`。
  - 保留批次 376 中“写入成功但刷新失败”的单独提示，不改变后端写入和审计副作用。
- `前端代码/src/pages/reconciliation/hooks/useReconciliationPage.test.ts`
  - 更新红绿断言，要求 BOM 修正成功后必须提示重新审计差异，以同步成本异常。

**ABC 影响评估**

- 本批只修改非 ABC 对账页面 hook 成功提示和前端测试，不修改 ABC 本体、ABC API、成本算法、库存、出库或废弃 `/cost-analysis`。
- BOM 修正会影响后续 BOM 理论消耗、对账差异和成本异常输入侧判断；本批不自动写入成本异常，不改变合法审计入口，只减少用户对异常台账同步状态的误判。
- 已补跑对账专项、出库和成本异常输入侧回归，确认不会破坏已完成的 ABC 成本透明化闭环。

**验证结果**

- 红灯验证:
  - `前端代码 npm test -- --run src/pages/reconciliation/hooks/useReconciliationPage.test.ts -t "does not report BOM correction"` 修复前失败：成功 toast 仍为 `BOM用量已修正`，没有提示重新审计成本异常。
- 修复后验证:
  - `前端代码 npm test -- --run src/pages/reconciliation/hooks/useReconciliationPage.test.ts -t "does not report BOM correction"` 通过。
  - `前端代码 npm test -- --run src/pages/reconciliation/hooks/useReconciliationPage.test.ts` 通过，17 tests passed。
  - `后端代码/server npm test -- --run tests/integration/reconciliation.test.ts tests/integration/outbound.test.ts tests/integration/cost-exceptions.test.ts` 通过，3 files / 62 tests passed；命令退出码为 0，保留既有 Vite close timeout 提示和 `cost-exceptions` 中模拟 `outbound_abc_details` 缺失的 stderr。
  ...
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
- 浏览器复核:
  - 本批为对账页面 hook 成功提示修复，不新增组件结构、路由或接口；核心风险是 BOM 修正成功后是否提示真实后续动作，已用 hook 级红绿测试覆盖 toast 结果，不新增截图证据。
...
- 人员效率统计卡片已阶段性保护为后端权威汇总口径；下一批继续处理人员效率跨筛选刷新，确认旧请求不能覆盖新筛选结果，避免报表读面回退到旧事实。

## 三百三十四、批次 379: 人员效率跨筛选刷新必须保留最新结果

**发现的问题**

- 本轮继续复核 ABC 之外的效率/报表展示页，聚焦“跨筛选刷新必须以最新请求结果为准”不变量。
- `PersonnelEfficiency` 页面在 `timeRange` 或 `role` 变化时会重新请求人员效率接口，但旧请求没有失效保护。
- 当用户快速切换筛选条件，且较早的请求晚于最新请求返回时，旧响应会重新写入 `ranking`、`trend` 和 `summary`，把页面卡片、排名和趋势图回退到旧筛选事实。
- 红灯测试模拟初始 `6m/all` 请求悬挂、用户切换到 `3m/all` 后新请求先返回人员数 2、旧请求随后返回人员数 9；修复前页面最终显示人员数 9 和旧人工成本，证明旧响应覆盖了新筛选结果。

**已完成修复**

- `前端代码/src/pages/cost/PersonnelEfficiency.tsx`
  - 新增 `latestRequestId`，每次 `loadData` 发起请求时递增并记录当前请求序号。
  - 接口成功返回时，仅当请求序号仍是最新序号才写入 `ranking`、`trend` 和 `backendSummary`。
  - 接口失败时，仅最新请求才弹出加载失败提示，避免旧请求失败误导用户。
  - `finally` 中仅最新请求可以关闭 loading，避免旧请求干扰新筛选加载状态。
- `前端代码/src/pages/cost/PersonnelEfficiency.render.test.tsx`
  - 新增红绿测试：模拟旧请求晚返回，断言页面仍保留最新筛选的人员数和人工成本。
  - 保留批次 378 的后端 `summary` 权威口径测试，确认两个读面保护点同时成立。

**ABC 影响评估**

- 本批只修改非 ABC 人员效率报表页面和前端测试，不修改 ABC 本体、ABC API、成本算法、库存、出库或废弃 `/cost-analysis`。
- 人员效率报表依赖出库、人员、项目类型和标准工时等历史事实；本批不改变后端汇总或成本输入，只保证前端跨筛选时不会把旧事实覆盖到新筛选读面。
- 已补跑人员效率后端专项和成本异常输入侧回归，确认不会破坏已完成的 ABC 成本透明化闭环。

**验证结果**

- 红灯验证:
  - `前端代码 npm test -- --run src/pages/cost/PersonnelEfficiency.render.test.tsx -t "keeps the latest filter"` 修复前失败：旧请求晚返回后页面显示 `人员数量9`，而不是最新筛选返回的 2。
- 修复后验证:
  - `前端代码 npm test -- --run src/pages/cost/PersonnelEfficiency.render.test.tsx` 通过，2 tests passed。
  - `后端代码/server npm test -- --run tests/integration/personnel-efficiency.test.ts tests/integration/cost-exceptions.test.ts` 通过，2 files / 17 tests passed；命令退出码为 0，保留既有 Vite close timeout 提示和 `cost-exceptions` 中模拟 `outbound_abc_details` 缺失的 stderr。
  - `后端代码/server npm run build` 通过。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
- 浏览器复核:
  - 本批为人员效率页面请求乱序保护，不新增路由、表单、弹窗或接口；核心风险是旧响应是否能覆盖新筛选读面，已用页面级红绿测试覆盖筛选操作、接口参数顺序和可见统计卡片，不新增截图证据。
...
- 盈利性分析导出月份和项目类型显示口径已阶段性保护；下一批转入切片成本明细项目类型读面，确认同类成本明细表格不泄露内部枚举。

## 三百三十八、批次 383: 切片成本明细项目类型必须使用业务显示口径

**发现的问题**

- 本轮按上一批后续风险继续复核切片成本明细页，聚焦“成本读面表格必须使用业务解释口径，而不是内部枚举”不变量。
- `SlideCostAnalysis` 的项目类型筛选器会显示 `HE染色 / 免疫组化 / 特殊染色` 等业务标签。
- 旧表格项目类型列直接渲染 `item.projectType`，导致 `HE检测` 所在行显示 `he`，与筛选器、盈利性分析表格和业务归档口径不一致。
- 页面级红灯测试还暴露 `SlideCostAnalysis.tsx` 与共享 `ProfitBadge.tsx` 在当前 Vitest/JSX 配置下缺少 React 导入，导致真实页面组件无法稳定渲染测试。

**已完成修复**

- `前端代码/src/pages/cost/SlideCostAnalysis.tsx`
  - 补充 React 导入，让页面在当前 Vitest/JSX 配置下可被页面级测试渲染。
  - 新增 `getProjectTypeLabel`，复用既有 `PROJECT_TYPE_OPTIONS` 把内部项目类型枚举转换成业务标签。
  - 表格项目类型列改为显示业务标签，未知类型保留原值回退。
  - 不改变列表加载、月份筛选、项目类型筛选、分页、导出参数、后端接口或成本聚合逻辑。
- `前端代码/src/components/ui/ProfitBadge.tsx`
  - 补充 React 导入，修复切片成本页首屏统计卡片和表格利润率徽章在页面级测试中的渲染前置问题。
- `前端代码/src/pages/cost/SlideCostAnalysis.test.ts`
  - 新增页面级红绿测试：接口返回 `projectType: 'he'` 时，`HE检测` 所在行必须显示 `HE染色`，且不能显示内部枚举 `he`。
  - 保留原有归一化测试，继续覆盖月份/项目类型过滤和物料/作业成本拆分。

**ABC 影响评估**

- 本批只修改非 ABC 切片成本明细页面的前端显示口径、共享利润率徽章的 React 导入和前端测试，不修改 ABC 本体、ABC API 后端、成本算法、库存、出库或废弃 `/cost-analysis`。
- 切片成本明细依赖既有出库 ABC 成本事实、BOM/项目类型字段和费用归集结果；本批不改变事实写入、出库扣减、成本异常、重算或导出数据，只提升页面读面的可解释性。
- 已补跑全成本和成本异常输入侧回归，确认不会破坏已完成的 ABC 成本透明化闭环。

**验证结果**

- 红灯验证:
  - `前端代码 npm test -- --run src/pages/cost/SlideCostAnalysis.test.ts -t "displays project type labels"` 首次暴露 `SlideCostAnalysis.tsx` 缺少 React 导入；补充后再次暴露 `ProfitBadge.tsx` 缺少 React 导入。
  - 清掉渲染前置问题后，同一命令按预期失败：`HE检测` 所在行显示内部枚举 `he`，找不到 `HE染色`。
- 修复后验证:
  - `前端代码 npm test -- --run src/pages/cost/SlideCostAnalysis.test.ts` 通过，2 tests passed。
  - `前端代码 npm test -- --run src/pages/cost/SlideCostAnalysis.test.ts src/pages/cost/ProfitabilityAnalysis.test.ts src/pages/cost/CostTrend.test.ts src/pages/cost/PersonnelEfficiency.render.test.tsx` 通过，4 files / 10 tests passed。
  - `后端代码/server npm test -- --run tests/integration/full-cost.test.ts tests/integration/cost-exceptions.test.ts` 通过，2 files / 14 tests passed；命令退出码为 0，保留既有 Vite close timeout 提示和 `cost-exceptions` 中模拟 `outbound_abc_details` 缺失的 stderr。
  - `后端代码/server npm run build` 通过。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
- 浏览器复核:
  - 本批为切片成本明细表格显示口径修复，不新增路由、表单、弹窗或接口；核心风险是可见行内容是否从内部枚举变为业务标签，已用页面级红绿测试覆盖接口数据、页面渲染和行内断言，不新增截图证据。
...
## 三百四十五、批次 390: 成本差异分析必须调用当前 ABC 差异接口并使用有效分组维度

**发现的问题**

- 本轮继续复核非废弃成本运营页中的成本差异分析，聚焦“页面筛选必须命中当前接口，列表必须按接口契约展示真实数据”不变量。
- `/abc/variance` 页面实际挂在当前 ABC 成本运营菜单下，后端当前接口为 `GET /api/v1/abc/variance-analysis`，返回 `{ summary, list }`。
- 前端旧 `CostVarianceAnalysis` 仍调用旧报表接口 `reportsApi.getCostVariance`，会把当前 ABC 差异页带回 `/api/v1/reports/cost-variance` 口径。
- 前端旧实现只读取 `items`，无法稳定渲染当前 `abc/variance-analysis` 返回的 `list`。
- 页面下拉仍保留旧报表的 `material` 维度；当前 `abc/variance-analysis` 支持项目、月份、BOM 维度，`material` 会落入后端默认项目分组，形成“看似按物料，实际按项目”的假筛选。
- BOM 维度下旧表格主标签固定显示 `projectName`，即便接口返回 `bomName` 也不能按 BOM 名称展示。

**已完成修复**

- `前端代码/src/pages/cost/CostVarianceAnalysis.tsx`
  - 将数据源从 `reportsApi.getCostVariance` 切换为 `abcApi.getVarianceAnalysis`。
  - 列表读取兼容当前接口的 `list`，同时保留 `items` 兼容兜底。
  - 分组下拉由 `project/month/material` 调整为 `project/month/bom`，避免无效 `material` 分组误导用户。
  - 新增行标识和主标签映射：项目维度显示项目名，月份维度显示月份，BOM 维度显示 `bomName`，展开状态也按 `id/bomId/month/projectId` 选择稳定 key。
  - 搜索占位和搜索字段随分组维度切换，BOM 维度按 BOM 名称搜索。
- `前端代码/src/pages/cost/CostVarianceAnalysis.render.test.tsx`
  - 新增当前接口红绿测试：页面必须调用 `abcApi.getVarianceAnalysis`，从 `list` 渲染项目差异行，并且不得调用旧 `reportsApi.getCostVariance`。
  - 新增 BOM 维度红绿测试：切换维度必须发送 `compareType: 'bom'`，行主标签必须显示 `bomName`，不得把项目名当作 BOM 维度主标签。
  - 原月份反向区间校验测试同步改为验证当前 `abcApi.getVarianceAnalysis` 调用不会发送非法日期范围。

**ABC 影响评估**

- 本批修改的是成本差异分析页面的数据源和维度展示契约；不改 ABC 成本算法、成本池重算、出库 ABC 明细、成本异常、收费映射、关账、库存/BOM/出库写链或废弃 `/cost-analysis`。
- 页面读取的 `abc/variance-analysis` 依赖已核算出库快照、BOM 和项目历史事实；本批只让页面读当前接口，不写入或重算任何 ABC 输入。
- 已补跑全成本和成本异常输入侧回归，确认不会破坏已完成的 ABC 成本透明化闭环。

**验证结果**

- 红灯验证:
  - `前端代码 npm test -- --run src/pages/cost/CostVarianceAnalysis.render.test.tsx` 首次按预期失败：旧组件没有调用 `abcApi.getVarianceAnalysis`，无法从 `list` 渲染 `胃癌筛查项目`。
  - 补充 BOM 维度红灯后再次运行同一目标测试按预期失败：下拉仍只有 `material`，无法显示 `HE染色BOM`。
- 修复后验证:
  - `前端代码 npm test -- --run src/pages/cost/CostVarianceAnalysis.render.test.tsx` 通过，3 tests passed。
  - `前端代码 npm test -- --run src/pages/cost/CostVarianceAnalysis.render.test.tsx src/pages/cost/QualityCostAnalysis.test.tsx src/pages/cost/BudgetManagement.test.tsx src/pages/cost/FeeComparison.test.tsx src/pages/cost/CostTrend.test.ts src/pages/cost/SlideCostAnalysis.test.ts src/pages/cost/ProfitabilityAnalysis.test.ts src/pages/cost/PersonnelEfficiency.render.test.tsx` 通过，8 files / 19 tests passed。
  ...
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
- 浏览器复核:
  - 使用用户指定 Chrome for Testing 路径启动 headless Playwright，前端 Vite 在 `127.0.0.1:8080`，注入本地 admin 登录态，拦截 `/api/v1/abc/variance-analysis`、`/api/v1/alerts` 和 `/api/v1/auth/me` 返回 `{ success: true, data }` 形状。
  - 页面 `/abc/variance` 复核结果：`hasPage=true`、初始项目行可渲染，切换 BOM 后 `showsBom=true`、`hidesProjectFallback=true`、`hasBomOption=true`、`hasMaterialOption=false`、请求序列包含 `compareType=project` 和 `compareType=bom`、`consoleErrors=[]`、`badResponses=[]`。
...
## 三百四十七、批次 392: 成本异常中心刷新失败不能保留旧异常数据

**发现的问题**

- 本轮继续复核非废弃成本运营页中的成本异常中心，聚焦“刷新失败时不能把上一次异常列表伪装成当前结果”不变量。
- 旧 `CostAlerts.loadExceptions` 在接口失败时只弹出 toast，不清空 `exceptions/summary/total`。
- 如果页面先显示历史异常，再刷新或切换筛选时请求失败，旧异常编号、旧异常说明和旧统计数字会继续留在屏幕上。
- 这会让成本异常处理人员误以为当前筛选条件仍存在这些异常，属于异常台账展示可信度问题。

**已完成修复**

- `前端代码/src/pages/cost/CostAlerts.tsx`
  - 补齐 `React` 默认导入，保证页面级 JSX 测试在当前测试环境下可直接渲染。
  - `loadExceptions` 的失败分支增加 `setExceptions([])`、`setSummary(normalizeExceptionSummary())` 和 `setTotal(0)`。
  - 保留原有 `toast.error('加载成本异常失败')`，但页面主体回到真实空态和零统计，避免旧异常残留。
- `前端代码/src/pages/cost/CostAlerts.render.test.tsx`
  - 新增红绿测试：首次加载显示 `CE-STALE` 和 `旧异常不能残留` 后，点击刷新并模拟接口失败，页面必须移除旧异常行、清空统计并显示 `暂无成本异常`。

**ABC 影响评估**

- 本批只修改成本异常中心页面错误态渲染；不改 ABC 成本算法、成本池重算、出库 ABC 明细、异常生成、异常处理接口、库存/BOM/出库写链或废弃 `/cost-analysis`。
- 页面失败态清空只影响前端展示，不写入、不回滚、不重试、不重算任何 ABC 输入或成本快照。
- 已补跑全成本和成本异常输入侧回归，确认不会破坏已完成的 ABC 成本透明化闭环。

**验证结果**

- 红灯验证:
  - `前端代码 npm test -- --run src/pages/cost/CostAlerts.render.test.tsx` 首次失败于旧页面缺少 `React` 默认导入，页面级 JSX 测试无法渲染。
  - 补齐导入后同一目标测试按预期失败：刷新失败后 `CE-STALE` 仍留在页面上，无法显示 `暂无成本异常`。
- 修复后验证:
  - `前端代码 npm test -- --run src/pages/cost/CostAlerts.render.test.tsx src/pages/cost/CostAlerts.test.ts` 通过，2 files / 8 tests passed。
  - `前端代码 npm test -- --run src/pages/cost/CostAlerts.render.test.tsx src/pages/cost/CostAlerts.test.ts src/pages/cost/CostDashboard.test.ts src/pages/cost/CostVarianceAnalysis.render.test.tsx src/pages/cost/QualityCostAnalysis.test.tsx src/pages/cost/BudgetManagement.test.tsx src/pages/cost/FeeComparison.test.tsx src/pages/cost/CostTrend.test.ts src/pages/cost/SlideCostAnalysis.test.ts src/pages/cost/ProfitabilityAnalysis.test.ts src/pages/cost/PersonnelEfficiency.render.test.tsx` 通过，11 files / 38 tests passed。
  - `后端代码/server npm test -- --run tests/integration/cost-exceptions.test.ts tests/integration/full-cost.test.ts` 通过，2 files / 14 tests passed；保留既有 Vite close timeout 提示和 `cost-exceptions` 中模拟 `outbound_abc_details` 缺失的 stderr。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `后端代码/server npm run build` 通过。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
- 浏览器复核:
  - 使用用户指定 Chrome for Testing 路径启动 headless Playwright，前端 Vite 在 `127.0.0.1:8080`，注入本地 admin 登录态。
  - `/api/v1/abc/exceptions` 前两次返回异常 `CE-STALE`，点击刷新后刻意返回 500，以验证失败态。
  - 页面 `/abc/alerts` 复核结果：`hasPage=true`、`initialExceptionVisible=true`、`emptyStateVisible=true`、`staleExceptionHidden=true`、`staleMessageHidden=true`、`statOnesCleared=true`、请求序列共 3 次且均带 `page=1&pageSize=20&status=open&yearMonth=2026-06`。
  - 本次浏览器证据中的 500 response 和 console resource error 是刻意模拟失败请求产生的预期现象；页面本身未新增崩溃。
...
## 三百五十一、批次 396: 关账后调整单审核成功后页面不能保留待审核旧状态

**发现的问题**

- 本轮按上一批后续风险继续复核成本看板中的关账后调整单，聚焦“写操作成功后页面状态必须反映真实副作用”不变量。
- 成本看板 `/abc/dashboard` 中的调整单审核按钮调用 `/api/v1/abc/adjustments/:id/approve` 或 `/reject` 成功后，只依赖后续整页 `loadDashboard()` 重拉数据刷新。
- 如果审核写操作已经成功，但后续看板刷新链路中的读请求失败，页面会保留原来的 `待审核` 行和“通过/驳回”按钮。
- 这会让用户误以为调整单尚未审核，并可能诱导重复点击或误判调整单是否已经进入财务结果修正链路。
- 页面级 JSX 测试还暴露 `CostDashboard.tsx` 缺少 React 默认导入，导致看板页面无法在当前测试运行时直接渲染。

**已完成修复**

- `前端代码/src/pages/cost/CostDashboard.tsx`
  - 补齐 React 默认导入，保证页面级 JSX 测试与运行时一致。
  - 新增本地合并逻辑：审核 API 成功返回调整单后，先把当前列表中的对应行合并为已通过/已驳回，再尝试重拉整页看板。
  - 新增 `applyReviewedAdjustmentToSummary`，在重拉失败时也同步减少待审核数；审核通过时同步计入调整金额、调整后总成本、调整后利润和利润率，驳回时不计入调整金额。
  - 正常情况下仍以 `loadDashboard()` 的后端结果为最终事实，不改变后端接口、ABC 成本算法或成本快照生成。
- `前端代码/src/pages/cost/CostDashboard.adjustments.render.test.tsx`
  - 新增页面级红绿测试：初始展示待审核调整单，点击通过后让后续看板刷新失败，断言页面仍显示已通过、审核人，并移除待审核状态和通过按钮。
- `前端代码/src/pages/cost/CostDashboard.test.ts`
  - 新增汇总合并单测，覆盖审核通过会减少待审核数并计入调整金额，审核驳回只减少待审核数。

**ABC 影响评估**

- 本批只修改成本看板前端状态刷新和测试，不改 ABC 成本算法、成本池重算、出库 ABC 明细、库存/BOM/出库写链、成本异常生成、调整单后端接口或废弃 `/cost-analysis`。
- 关账后调整单审核结果会影响看板汇总和导出结果；本批只在审核 API 已成功返回后同步页面状态，避免前端残留旧状态，不新增写入、不重算、不回滚任何成本事实。
- 已补跑成本异常和全成本输入侧回归，确认调整单后端审核、出库 ABC 快照、成本异常处理和全成本报表输入链仍稳定。

**验证结果**

- 红灯验证:
  - `前端代码 npm test -- --run src/pages/cost/CostDashboard.adjustments.render.test.tsx` 首次失败于旧页面缺少 `React` 默认导入，页面级 JSX 测试无法渲染。
  - 补齐导入后同一目标测试按预期失败：审批 API 成功、后续刷新失败时，页面仍保留 `待审核` 状态，新增测试期望该状态消失并显示 `已通过`。
- 修复后验证:
  - `前端代码 npm test -- --run src/pages/cost/CostDashboard.adjustments.render.test.tsx` 通过，1 file / 1 test passed。
  - `前端代码 npm test -- --run src/pages/cost/CostDashboard.adjustments.render.test.tsx src/pages/cost/CostDashboard.test.ts` 通过，2 files / 13 tests passed。
  ...
  - `后端代码/server npm test -- --run tests/integration/cost-exceptions.test.ts tests/integration/full-cost.test.ts` 通过，2 files / 14 tests passed；保留既有 Vite close timeout 提示和模拟 `outbound_abc_details` 缺失的 stderr。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `后端代码/server npm run build` 通过。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
- 浏览器复核:
  ...
  - 页面 `/abc/dashboard` 初始返回一张 `ADJ-209906-001` 待审核调整单；点击通过后，审批接口返回已通过，随后刻意让看板刷新返回 500。
  - 复核结果：操作前 `pendingVisible=true`、`approveButtonVisible=true`；操作后 `approvedVisible=true`、`reviewerVisible=true`、`pendingHidden=true`、`approveButtonHidden=true`、`approveCount=1`。
  - 本次浏览器证据中的 500 response 和 console resource error 是刻意模拟刷新失败请求产生的预期现象；页面本身未新增崩溃。
...
## 三百五十六、批次 401: 操作日志列表分页参数必须可解释

**发现的问题**

- 本轮继续按第二阶段库存主链路的“库存日志和报表读取面”推进，聚焦“分页参数必须是可解释正整数，不能把错误参数静默变成正常查询”不变量。
- 操作日志列表 `/api/v1/logs` 已能校验日期范围、操作类型、模块筛选和导出筛选，但列表分页仍使用 `Math.max/Math.min/Number` 静默兜底。
- `page=0` 会被改成第 1 页，`page=abc` 会让分页元数据不可解释，`pageSize=0` 会被改成 1，`pageSize=10001` 会被截断成 10000。
- 这会把 URL、集成调用或手工请求中的参数错误伪装成正常日志结果，削弱库存日志、审计追踪和报表读取面的可解释性。

**已完成修复**

- `后端代码/server/src/routes/logs-v1.1.ts`
  - 新增日志列表分页解析校验，默认仍为 `page=1&pageSize=20`。
  - 显式传入分页时必须满足 `page >= 1` 且为整数，`pageSize` 必须为 `1-10000` 的整数。
  - 非法分页返回 `400 INVALID_PARAMETER`，不再静默兜底或截断。
  - 合法分页继续使用原有列表、总数和响应结构，不影响日志导出、统计或清理接口。
- `后端代码/server/tests/logs.test.ts`
  - 新增 `LOG-012` 红绿测试，覆盖 `page=0`、非数字页码、小数页码、`pageSize=0`、非数字 pageSize 和超过上限 pageSize。

**ABC 影响评估**

- 本批只修改非 ABC 操作日志列表的只读分页校验，不改 ABC 成本算法、成本池重算、出库 ABC 明细、库存/BOM/出库写链、成本异常生成或废弃 `/cost-analysis`。
- 操作日志是库存、出库、盘点、报表和审计追踪的说明面；本批不写入、不重算、不回滚任何库存或成本输入，只阻断错误分页参数伪装成正常结果。
- 已补跑库存一致性、库存列表、盘点、出库、非 ABC 报表和成本异常输入侧回归，确认库存/出库/报表/ABC 输入链未被破坏。

**验证结果**

- 红灯验证:
  - `后端代码/server npm test -- --run tests/logs.test.ts -t "LOG-012"` 修复前失败：`page=0&pageSize=20` 返回 200，证明错误分页会被静默改写。
- 修复后验证:
  - `后端代码/server npm test -- --run tests/logs.test.ts -t "LOG-012"` 通过，1 test passed / 11 skipped。
  - `后端代码/server npm test -- --run tests/logs.test.ts` 通过，1 file / 12 tests passed。
  - `后端代码/server npm test -- --run tests/inventory-consistency.test.ts tests/integration/inventory.test.ts tests/stocktaking.test.ts tests/integration/outbound.test.ts tests/integration/reports-date-validation.test.ts tests/integration/reports-cost-by-material.test.ts tests/integration/reports-cost-by-supplier.test.ts tests/integration/reports-cost-by-project-group.test.ts tests/integration/cost-exceptions.test.ts` 通过，9 files / 94 tests passed；保留既有 Vite close timeout 提示和模拟 `outbound_abc_details` 缺失的 stderr。
  - `后端代码/server npm run build` 通过。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
...
- 操作日志列表的日期、类型、模块、导出筛选和分页异常语义已阶段性收口；下一批可继续按计划复核库存/报表读取面中剩余来源、日期、导出口径或回到第一阶段基础资料尚未当前复核的 P0/P1 项，不在本批继续扩展。

## 三百五十七、批次 402: 库存列表分页参数必须可解释

**发现的问题**

- 本轮继续按第二阶段库存主链路的库存读取面推进，聚焦“分页参数必须是可解释正整数，不能把错误参数静默变成正常库存结果”不变量。
- 库存列表 `/api/v1/inventory` 已能校验非法状态、分类/库位/物料筛选来源和统计口径，但分页仍使用 `Math.max/Math.min/Number` 静默兜底。
- `page=0` 会被改成第 1 页，`page=abc`、小数页码、`pageSize=0`、非数字 pageSize 或超过上限 pageSize 都没有统一业务错误。
- 这会把 URL、集成调用或手工请求中的参数错误伪装成正常库存列表，影响库存总账、批次/库位说明面和出库候选判断的可解释性。

**已完成修复**

- `后端代码/server/src/routes/inventory-v1.1.ts`
  - 新增库存列表分页解析校验，默认仍为 `page=1&pageSize=20`。
  - 显式传入分页时必须满足 `page >= 1` 且为整数，`pageSize` 必须为 `1-200` 的整数，沿用原列表最大页大小。
  - 非法分页返回 `400 INVALID_PARAMETER`，不再静默兜底或截断。
  - 合法分页继续使用原有列表、总数和响应结构，不影响库存统计、库存一致性扫描或任何库存写入。
- `后端代码/server/tests/integration/inventory.test.ts`
  - 新增 `INV-PAGE-001` 红绿测试，覆盖 `page=0`、非数字页码、小数页码、`pageSize=0`、非数字 pageSize 和超过上限 pageSize。

**ABC 影响评估**

- 本批只修改非 ABC 库存列表只读分页校验，不改 ABC 成本算法、成本池重算、出库 ABC 明细、BOM、出库扣减、成本异常生成或废弃 `/cost-analysis`。
- 库存列表是库存总账、批次余额、库位明细和出库候选判断的说明面；本批不写入、不重算、不回滚任何库存或成本输入，只阻断错误分页参数伪装成正常结果。
- 已补跑库存一致性、操作日志、盘点、出库和成本异常输入侧回归，确认库存/出库/异常/ABC 输入链未被破坏。

**验证结果**

- 红灯验证:
  - `后端代码/server npm test -- --run tests/integration/inventory.test.ts -t "INV-PAGE-001"` 修复前失败：`page=0&pageSize=20` 返回 200，证明错误分页会被静默改写。
- 修复后验证:
  - `后端代码/server npm test -- --run tests/integration/inventory.test.ts -t "INV-PAGE-001"` 通过，1 test passed / 17 skipped。
  - `后端代码/server npm test -- --run tests/integration/inventory.test.ts` 通过，1 file / 18 tests passed。
  - `后端代码/server npm test -- --run tests/inventory-consistency.test.ts tests/logs.test.ts tests/stocktaking.test.ts tests/integration/outbound.test.ts tests/integration/cost-exceptions.test.ts` 通过，5 files / 74 tests passed；保留既有 Vite close timeout 提示和模拟 `outbound_abc_details` 缺失的 stderr。
  - `后端代码/server npm run build` 通过。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
...
## 三百六十、批次 405: 出库列表来源筛选必须可解释

**发现的问题**

- 本轮继续按第二阶段库存主链路的库存/报表读取面推进，聚焦“来源和枚举筛选必须可解释，不能把错误来源静默伪装成空结果”不变量。
- 出库列表 `/api/v1/outbound` 已在上一批补齐日期校验，但 `projectId/materialId/status/type` 仍直接进入查询条件。
- `status=archived`、`type=mystery`、不存在检测项目或不存在物料会返回 200 空列表，而不是明确业务错误。
- 这会让 URL、集成调用或手工请求中的错误筛选被误认为“确实没有出库记录”，影响出库历史检索、库存流水解释和 ABC 上游事实核对。

**已完成修复**

- `后端代码/server/src/routes/outbound-v1.1.ts`
  - 新增出库列表筛选校验。
  - `status` 仅允许 `completed/pending/cancelled`。
  - `type` 仅允许普通出库类型 `project/transfer/scrap` 和 BOM 出库类型 `bom`。
  - `projectId/materialId` 必须指向未删除检测项目或物料；不存在时返回 `400 INVALID_PARAMETER`。
  - 校验在 SQL 查询前执行；合法筛选、列表响应结构、出库创建、BOM 出库、编辑和删除链路不变。
- `后端代码/server/tests/integration/outbound.test.ts`
  - 新增 `OUT-FILTER-001` 红绿测试，覆盖非法状态、非法类型、不存在项目和不存在物料筛选。

**ABC 影响评估**

- 本批只修改非 ABC 出库列表只读筛选校验，不改 ABC 成本算法、成本池重算、出库 ABC 明细、BOM、出库扣减、成本异常生成或废弃 `/cost-analysis`。
- 出库是 ABC 上游核心输入之一；本批不写入、不重算、不回滚任何库存或成本输入，只阻断错误筛选来源伪装成正常出库结果。
- 已补跑库存一致性、库存列表、操作日志、非 ABC 报表日期和成本异常输入侧回归，确认库存/出库/报表/异常/ABC 输入链未被破坏。

**验证结果**

- 红灯验证:
  - `后端代码/server npm test -- --run tests/integration/outbound.test.ts -t "OUT-FILTER-001"` 修复前失败：`status=archived` 返回 200，证明错误枚举会被静默当成正常查询。
- 修复后验证:
  - `后端代码/server npm test -- --run tests/integration/outbound.test.ts -t "OUT-FILTER-001"` 通过，1 test passed / 30 skipped。
  - `后端代码/server npm test -- --run tests/integration/outbound.test.ts` 通过，1 file / 31 tests passed。
  - `后端代码/server npm test -- --run tests/integration/outbound.test.ts tests/inventory-consistency.test.ts tests/integration/inventory.test.ts tests/logs.test.ts tests/integration/reports-date-validation.test.ts tests/integration/cost-exceptions.test.ts` 通过，6 files / 81 tests passed；保留既有 Vite close timeout 提示和模拟 `outbound_abc_details` 缺失的 stderr。
  - `后端代码/server npm run build` 通过。
  - `前端代码 npm run build` 通过，保留既有 chunk size warning。
  - `git diff --check` 通过。
  - `git diff --name-only -- 前端代码/deprecated/legacy-cost-analysis` 无输出，确认未改废弃范围。
