...
| Draft PR 创建 | 已完成。PR #1 已打开，当前 head 为 `3f66b93e33bdb3517f81f7f9b1474ae62fc218b6` |
...
| CI 启动修复 | 已为 Playwright 后端 webServer 注入 E2E 专用 JWT 环境变量，GitHub E2E 已越过后端启动失败 |

## 二、仍未闭环事项

| 优先级 | 事项 | 当前状态 | 下一步 |
|--------|------|----------|--------|
| P0 | GitHub Actions `e2e` | 失败：256/257 通过，唯一失败为 `BF-PERM-technician-outbound` | PM 明确 technician 是否允许访问出库；若不允许，需同步调整前端守卫/菜单和后端权限 |
...
| P1 | 权限差异 | 路由缺失已修复；CI 暴露 technician 出库权限与 E2E 预期冲突 | PM 确认角色可见性、technician 出库权限和隐藏 ABC 页面策略 |
...
| `docs/08_Test_Cases_PM.md` | 标明 PR E2E 256/257，通过项不能覆盖唯一失败项 |
...
1. PM 明确 technician 是否应访问 `/outbound`，并按结论修复代码或调整测试预期。
