...
### 9.7 成本归集与披露约束（ABC 核算口径）

> **⚠️ 定位说明**：本节是 **ABC 成本法矫正的目标规则**，依据 `docs/COREONE-成本核算事实源说明-2026-06-24.md` 与 `plans/abc-full-chain-correction-2026-06-24.md`（B 方案）。与本文档其余"以代码为准"的规则不同，本节规则**部分尚未在代码实现**：现状 `autoCollectCostPools`（abc-v1.1.ts:457-483）仍按作业中心个数平均分摊（违反 BR-BM-024/026），将于矫正计划 M2/M3 落地。本节细化并升格 `BR-BM-017`（人工按项目类型）、`BR-BM-020`（间接按月份）的笼统口径，并与出库实际成本 `BR-OB-028` 桥接。

| 编号 | 规则 | 说明 |
|------|------|------|
| BR-BM-024 | 成本来源**按作业中心动因归集**：每条人工步骤、每台设备、每个间接成本来源声明归属作业中心；每中心 `池成本 = Σ本中心人工 + Σ本中心设备 + 本中心分得间接`，`动因费率 = 池成本 / 该中心动因当期实际总量` | 升格 `BR-BM-017/020`；**禁止**按中心个数平均分摊或用全局样本量做动因量（CHAIN-04） |
| BR-BM-025 | **间接费单一披露基准**：间接费天然不可干净追溯，全期按**一个系统级公开声明的单一基准**分到各中心，默认 `by_direct_cost`（各中心直接成本占比）；同一期间只用一个基准 | 候选集 `by_direct_cost`/`by_driver_volume`/`by_slide_equivalent`；UI 须标注"间接分摊估算（基准：X）"（CHAIN-09） |
| BR-BM-026 | **完全吸收约束**：`Σ各中心池成本` 必须等于 `本期人工 + 设备 + 间接总额`，无遗漏无重复；差额写入 `cost_exceptions` 异常台账 | 锚定 `BR-BM-014` 五要素；对账容差仅四舍五入级（CHAIN-06） |
| BR-BM-027 | **可解释约束**：任一切片成本必须能还原为逐作业中心的 `{动因类型, 动因量, 费率, 费率出处}`；无法解释的成本不得静默产出，须进 `cost_exceptions` | 切片成本下钻可见（CHAIN-07） |
| BR-BM-028 | **标准成本（计划值）vs 实际成本（核算值）区分**：标准成本来自 BOM `standard_*`（见 `BR-BM-014`），用于定价/预算；实际成本来自期间成本池 + 实际动因（见 `BR-OB-028`），用于核算/关账/利润；差异 = 标准 − 实际。任一成本对外展示**必须标注口径** | 杜绝同一标签下两个数打架（CHAIN-08） |
