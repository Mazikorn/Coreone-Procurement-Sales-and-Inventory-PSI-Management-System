...
> ⚠️ **本文件当前只在 codex/abc 线，不在 master**——在 master/phase2 等其它 worktree 开的会话物理上读不到这份 P0 治理规则。收口方案：随下一个落 master 的 PR 把 `.claude/rules/pr-governance.md` 带上 master（见调研报告 §〇.B）。
